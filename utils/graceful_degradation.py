...
class GracefulDegradationManager:
    ...
    def __init__(self, server_monitor: ServerHealthMonitor | None = None):
        ...
        self.workflow_patterns = {
            "research_workflow": [
                ...
                WorkflowStep(
                    "create_note", required=True, degradation_strategy=DegradationStrategy.FAIL
                ),
                WorkflowStep(
                    "todoist_create_task",
                    required=False,
            ...
            "information_synthesis": [
                ...
                WorkflowStep(
                    "web_url_read", required=False, degradation_strategy=DegradationStrategy.SKIP
                ),
                WorkflowStep(
                    "create_note", required=True, degradation_strategy=DegradationStrategy.FAIL
                ),
    ...
    def plan_workflow_with_degradation(
        self, intended_workflow: list[str]
    ...
    def _find_alternative(
        self, unavailable_tool: str, available_tools: set[str]
    ) -> tuple[str | None, str]:
        """
    ...
    def execute_with_degradation(
        self, workflow_pattern: str, user_query: str
                ...
                if step.degradation_strategy == DegradationStrategy.FAIL and step.required:
                    ...
                    if not tool_executed and step.required:
                        return (
                            False,
                            executed_tools,
                            f"No alternatives available for required tool {step.tool_name}",
                        )
                    ...
                    degradation_messages.append(
                        f"Continuing with reduced functionality (missing {step.tool_name})"
                    )
                    continue
        ...
        result_parts = [
            f"Executed workflow '{workflow_pattern}' with tools: {', '.join(executed_tools)}"
        ]
    ...
    def add_tool_alternative(
        self, primary: str, alternative: str, capability_match: float, notes: str = ""
    ):
        """
